...
public class PagingDto {
    private int pageSize;   // 화면 당 보여지는 게시글 최대갯수
    private int totalPageNum;   // 총페이지 수
    ...
    // 전체 리스트 크기와 현제 페이지와 페이지마다 나타낼 글 갯수, 블럭수를 가지고 나머지
    // 필요 변수들의 값을 계산하는 생성자
    public PagingDto(Long totalListSize, Integer page, Integer pageSize, Integer blockSize) {
        ...
        // 블럭 마지막페이지 검증 (한 블럭이 10페이지가 안 넘으면 마지막페이지를 최대 페이지수로 다시변경 10 -> 3)
